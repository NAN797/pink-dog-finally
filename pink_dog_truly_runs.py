...
if st.button(tr("开始拟合", "Fit Now")):
    try:
        ...
        if len(E_values) != len(D_values):
            ...
            st.markdown(f"**{tr('阈值能量 E_th_val', 'Threshold energy E_th_val')}** ≈ `{E_th_val:.2f}` μJ")
            st.markdown(f"**{tr('光斑半径 w₀', 'Beam waist w₀')}** ≈ `{w0_val:.2f}` μm")
            st.markdown(f"**{tr('阈值通量 F_th_val', 'Fluence threshold F_th_val')}** ≈ `{F_th_val:.5f}` J/cm²")
            ...
            with pd.ExcelWriter(towrite, engine='xlsxwriter') as writer:
                ...
                pd.DataFrame({
                    'E_th_val (μJ)': [E_th_val],
                    'w0_val (μm)': [w0_val],
                    'F_th_val (J/cm²)': [F_th_val],
                    'R²': [r_value**2]
