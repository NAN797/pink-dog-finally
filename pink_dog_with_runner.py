...
<img class="running-dog" src="https://cdn-icons-png.flaticon.com/512/616/616408.png" alt="dog running">
...
if st.button(tr("开始拟合", "Fit Now")):
    try:
        ...
        if len(E_values) != len(D_values):
            ...
            st.markdown(f"**{tr('阈值能量 E_th', 'Threshold energy E_th')}** ≈ `{E_th_val:.2f}` μJ")
            st.markdown(f"**{tr('光斑半径 w₀', 'Beam waist w₀')}** ≈ `{w0_val:.2f}` μm")
            st.markdown(f"**{tr('阈值通量 F_th', 'Fluence threshold F_th')}** ≈ `{F_th_val:.5f}` J/cm²")
        ...
        st.error(f"发生错误 / Error occurred: {e}")
